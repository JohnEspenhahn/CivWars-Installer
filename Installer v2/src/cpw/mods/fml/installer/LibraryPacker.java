...
public class LibraryPacker {
    ...
    private static void walk(File path) throws IOException {
        if (path.isDirectory()) {
            ...
            LibraryPacker.decompress(path, xz, LibraryPacker.CHECKSUMS);
        }
    }
    ...
    private static byte[] compress(File path) throws IOException {
        ...
        LibraryPacker.CHECKSUMS.add(Hashing.sha1().hashBytes(checksums).toString());
    ...
    private static byte[] pack(byte[] data, File path) throws IOException {
        ...
        System.setErr(new PrintStream(LibraryPacker.NULL_OUT));
        packer.pack(in, out);
        System.setErr(err);
    ...
    private static void decompress(File path, byte[] data, List<String> checksum) throws IOException {
        ...
        DownloadUtils.validateJar(output, Files.toByteArray(output), LibraryPacker.CHECKSUMS);
    ...
    private static class Stopwatch {
        ...
        public void start() {
            if (this.fIsRunning) throw new IllegalStateException("Must stop before calling start again.");
        ...
        public void stop() {
            if (!this.fIsRunning) throw new IllegalStateException("Cannot stop if not currently running.");
...
/*
 * Location: C:\Users\John\Desktop\forge-1.6.4-9.11.1.953-installer (1).jar Qualified Name: cpw.mods.fml.installer.LibraryPacker JD-Core Version:
 * 0.6.2
 */